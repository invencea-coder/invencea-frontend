...
import { useAuth } from '../../context/AuthContext.jsx'; // Updated path if needed
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    // List requests for the logged-in student
    listRequests({})
  ...
  const { activeRequests, history, overdueCount } = useMemo(() => {
    ...
    // Sort active so APPROVED (ready for pickup) is at the top!
    active.sort((a, b) => {
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {activeRequests.map(r => (
              <div key={r.id} className={`p-4 rounded-2xl border bg-white shadow-sm transition-all ${r.isOverdue ? 'border-red-300 shadow-red-500/10' : r.status === 'APPROVED' ? 'border-emerald-300 shadow-emerald-500/10' : 'border-black/5'}`}>
                
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <span className={`inline-block px-2 py-0.5 text-[9px] font-black rounded uppercase tracking-wider border ${getStatusBadge(r.status)}`}>
                      {r.status}
                    </span>
            ...
            {history.slice(0, 3).map((r, idx) => (
              <div key={r.id} className={`p-4 flex items-center justify-between ${idx !== 0 ? 'border-t border-black/5' : ''}`}>
                ...
                <div className="flex flex-col items-end">
                  <span className={`px-2 py-0.5 text-[9px] font-black rounded uppercase tracking-wider border ${getStatusBadge(r.status)}`}>{r.status}</span>
