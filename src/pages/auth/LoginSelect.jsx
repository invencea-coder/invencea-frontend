...
const StatusBadge = ({ status }) => {
  ...
  return <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase border ${colorClass}`}>{status}</span>;
};
...
export default function LoginSelect() {
  ...
  // Handle the Bluetooth Scanner's input
  const handleScannerInput = async (e) => {
    // The scanner types the code and fires the 'Enter' key instantly
    if (e.key === 'Enter') {
      ...
      try {
        ...
        // NOTE: Make sure you have a public endpoint for this! (See instructions below)
        const res = await api.get(`/public/qr-status/${code}`);
        setScanResult(res.data.data || res.data);
        toast.success('Request found!');
      } catch (err) {
        toast.error('Invalid QR Code or Request not found.');
        // Refocus so they can try again
        setTimeout(() => scannerInputRef.current?.focus(), 100);
      } finally {
        setIsScanning(false);
      }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-surface dark:bg-darkSurface px-4">
      <div className="animate-slide-up w-full max-w-md">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-sm text-muted dark:text-darkMuted mt-1">Inventory System of College of Engineering and Architecture</p>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted dark:text-darkMuted mt-0.5">Scan your QR code using the scanner</p>
            </div>
        ...
        <footer className="mt-auto py-8 text-center">
          <p className="text-xs text-muted dark:text-darkMuted font-display tracking-wide">
            &copy; {new Date().getFullYear()} <span className="font-bold text-primary dark:text-darkText">InvenCEA</span>. All rights reserved.
          </p>
        </footer>
      </div>
      ...
      {qrModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
          <div className="neu-card-lg w-full max-w-md relative bg-white dark:bg-darkSurface overflow-hidden">
            ...
            {/* Close Button */}
            <button onClick={closeModal} className="absolute top-4 right-4 p-2 text-muted hover:text-red-500 rounded-full transition-colors z-10">
              <X size={20} />
            </button>
            ...
            <input 
              ...
              // Force focus back if they accidentally click elsewhere while modal is open
              ...
              <div className="flex flex-col items-center justify-center p-8 text-center space-y-6">
                <div className="relative">
                  ...
                  <div className="w-24 h-24 neu-inset rounded-full flex items-center justify-center relative z-10 bg-white">
                    {isScanning ? <Loader2 size={40} className="text-primary animate-spin" /> : <ScanLine size={40} className="text-primary" />}
                  </div>
                </div>
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="neu-inset p-4 rounded-xl space-y-3 text-sm">
                  <div className="flex justify-between items-center">
                    <span className="text-muted font-semibold flex items-center gap-1"><Clock size={14}/> Status</span>
                ...
                <button onClick={() => setScanResult(null)} className="w-full neu-btn text-primary py-3 font-bold mt-4">
                  Scan Another Code
                </button>
              </div>
            )}
