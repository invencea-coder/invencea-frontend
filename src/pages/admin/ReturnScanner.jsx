...
import { ScanBarcode, Camera, Search, Loader2, CheckCircle, XCircle } from "lucide-react";
...
export default function ReturnScanner() {
  ...
  // Instead of immediately processing, we stage the scanned barcode for confirmation
  const [stagedBarcode, setStagedBarcode] = useState("");   // waiting for confirm
  const [scannedBarcode, setScannedBarcode] = useState(""); // last successfully returned
      ...
      requests.forEach((req) => {
        ...
        if (isMyRoom) {
          (req.items || []).forEach((item) => {
            const barcode = item.inventory_item_barcode || item.barcode || item.barcode_text || item.item_barcode;
  ...
  useEffect(() => {
    if (!authLoading) fetchActiveItems();
  }, [authLoading, fetchActiveItems]);

  // Called by scanner/keyboard — stages the barcode, does NOT process yet
  const stageBarcode = (barcodeArg) => {
    const barcode = (barcodeArg ?? "").toString().trim();
    if (!barcode || isProcessing) return;
    setStagedBarcode(barcode);
  };

  // Called only when user clicks "Confirm Return"
  const confirmReturn = async () => {
    ...
    try {
      const res = await returnItemByBarcode({ barcode, condition: conditionRef.current });
  ...
  // Global keyboard listener — now stages instead of processing
  useEffect(() => {
    const handleGlobalKeyDown = (e) => {
      ...
      if (timeDiff > 50) scannerBufferRef.current = "";
  ...
  // Camera scanner — now stages instead of processing
  useEffect(() => {
    ...
    const stopAndClear = async () => {
      ...
      if (!inst) return;
      try { if (typeof inst.stop === "function") await inst.stop(); } catch (e) {}
      try { if (typeof inst.clear === "function") await inst.clear(); } catch (e) {}
    ...
    const startScanner = async () => {
      try {
        ...
        await html5QrCode.start(
          ...
          async (decodedText) => {
            try { if (typeof html5QrCode.stop === "function") await html5QrCode.stop(); } catch (e) {}
            try { if (typeof html5QrCode.clear === "function") await html5QrCode.clear(); } catch (e) {}
            scannerRef.current = null;
            localStoppedByUs = true;
            setCameraActive(false);
  ...
  // Find item details for staged barcode to show in confirmation panel
  const stagedItemInfo = stagedBarcode
    ? activeItems.find((i) => i.barcode === stagedBarcode) || null
    : null;
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Left Column: Scanner, Condition & Confirm */}
        <div className="lg:col-span-1 space-y-4">
          <NeumorphCard className="p-6 space-y-6">
                  ...
                  <button
                    ...
                    className={`p-2 rounded-xl border-2 text-[10px] uppercase font-bold tracking-wider transition-all ${
                      condition === label
                        ...
                        : "border-transparent bg-black/[0.02] text-muted hover:bg-black/5"
                    }`}
                  >
                    {label}
                  </button>
            ...
            {/* Confirmation Panel — shown after a barcode is staged */}
            <div className={`pt-4 border-t border-black/5 space-y-3 transition-all`}>
                ...
                <div className="rounded-2xl border-2 border-primary/30 bg-primary/5 p-4 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="space-y-0.5">
                      <p className="font-mono font-bold text-primary text-lg leading-none">{stagedBarcode}</p>
                      {stagedItemInfo ? (
                        <>
                          <p className="text-sm font-semibold text-gray-800 mt-1">{stagedItemInfo.itemName}</p>
                        ...
                        <p className="text-xs text-amber-600 mt-1">⚠ Barcode not in active list — proceed carefully</p>
                      )}
                    </div>
                    <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded-lg flex-shrink-0 ${
                      ...
                      condition === "Damaged" ? "bg-amber-100 text-amber-700" :
                  ...
                  <div className="flex gap-2 pt-1">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl border-2 border-black/10 text-xs font-bold text-muted hover:bg-black/5 transition-colors disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      disabled={isProcessing}
                      className="flex-2 flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-primary text-white text-xs font-bold hover:bg-primary/90 transition-colors disabled:opacity-60"
                    >
                      {isProcessing ? (
            ...
            <div className="p-4 bg-black/5 border-b flex justify-between items-center">
              ...
              <div className="relative w-48 lg:w-64">
                ...
                <input
                  ...
                  className="neu-input w-full pl-8 text-xs py-1.5"
                  value={searchQuery}
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {filteredItems.map((item, i) => (
                      <tr
                        key={i}
                        onClick={() => stageBarcode(item.barcode)}
                        className={`border-b border-black/5 cursor-pointer transition-colors ${
                          stagedBarcode === item.barcode
                            ? "bg-primary/10"
                            : "hover:bg-black/[0.02]"
                        }`}
                      >
                        <td className="px-4 py-3 font-mono font-bold text-primary">{item.barcode}</td>
                        ...
                        <td className="px-4 py-3 text-xs text-right text-muted">#{item.requestId}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
