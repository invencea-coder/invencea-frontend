...
import { 
  ...
  Search, Clock, CheckCircle2, AlertCircle, Users, Box, Plus, Minus, Trash2,
  Copy, Check, X, Barcode, History, Link as LinkIcon, Lock, FlaskConical,
} from 'lucide-react';
...
export default function AdminRequests() {
  ...
  const load = async (silent = false) => {
    ...
    try {
      ...
      const enrichedData = reqRes.data.data.map(r => ({
        ...r,
        isExpired: isExpiredSameDay(r.created_at) && ['PENDING', 'APPROVED', 'PENDING APPROVAL'].includes(r.status),
      }));
  ...
  const filteredRequests = useMemo(() => {
    return requests.filter(r => {
      ...
      const matchesSearch = !q
        ...
        || String(r.requester_id).toLowerCase().includes(q)
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      load(true);
    } catch { toast.error('Approval failed', { id: 'action' }); }
  };
  ...
  const handleReject = async (id) => {
    ...
    try {
      ...
      load(true);
    } catch { toast.error('Rejection failed', { id: 'action' }); }
  };
  ...
  const handleQRResult = useCallback(async (code) => {
    ...
    try {
      ...
      if (['PENDING', 'PENDING APPROVAL'].includes(req.status)) {
        // Lab session requests shouldn't hit PENDING, but handle gracefully
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'Enter') {
        ...
        if (code.length > 0) {
          ...
          else if (!isRoomLocked) handleQRResult(code);
        }
        return;
      }
  ...
  const handleManualSync = async () => {
    ...
    try {
      ...
      setManualForm({ studentId: '', purpose: 'Manual / Offline Log', manualSearch: '' });
      setManualItems([]);
      load(true);
    } catch (e) {
            ...
            {!item.consumable_id && !item.isQtyMode && item.actualQuantity > 0 && (
              <div className="mt-2 flex flex-wrap gap-1.5 items-center">
                ...
                {validInventory.length > 0 ? validInventory.map(inv => {
                  ...
                  return (
                    <span key={inv.id} className={`text-[10px] font-mono px-1.5 py-0.5 rounded transition-all ${isScanned ? 'bg-green-100 text-green-700 line-through opacity-50' : 'bg-black/5 text-gray-700'}`}>
                      {String(inv.barcode)}
                    </span>
                  );
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <button
            ...
            className={`flex items-start gap-3 p-4 w-full text-left bg-surface border border-black/10 rounded-xl transition-all group ${isRoomLocked ? 'opacity-50 grayscale cursor-not-allowed' : 'hover:border-primary/30 hover:shadow-md'}`}
          >
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs text-muted mt-0.5 leading-relaxed">
                ...
                <span className="text-red-800 font-bold">Borrower's slip</span> here.
              </span>
            </div>
          </button>
        </div>
                ...
                <NeumorphCard key={r.id} className={`p-0 overflow-hidden transition-all ${r.isExpired ? 'opacity-70 grayscale' : ''}`}>
                  ...
                  >
                    <div className="flex items-center gap-4 flex-1">
                      <div className="w-12 h-12 rounded-xl bg-black/5 flex items-center justify-center font-mono font-bold text-primary">
                        #{r.id}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {['PENDING', 'PENDING APPROVAL'].includes(r.status) && !r.isExpired && (
                        <div className="hidden sm:flex items-center gap-2 mr-4">
                          {String(r.requester_type).toLowerCase() === 'faculty' ? (
                            ...
                            // Lab session — already pre-approved, no action needed at PENDING stage
                            <span className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-emerald-100 text-emerald-700">
                              <FlaskConical size={13} /> Pre-approved by Faculty
                            </span>
                          ) : (
                            <button
                              disabled={isRoomLocked}
                              onClick={(e) => { e.stopPropagation(); setCameraModal(true); }}
                      ...
                      {r.status === 'APPROVED' && !r.isExpired && (
                        ...
                        >
                          {isRoomLocked ? <Lock size={13} className="mr-1.5" /> : <Scan size={13} className="mr-1.5" />}
                          Verify & Issue
                        </NeumorphButton>
                      )}
                      ...
                      <div className="space-y-4">
                        ...
                        {r.lab_session_id && (
                          <div className="bg-emerald-50 border border-emerald-200 p-3 rounded-xl">
                            <p className="text-xs font-bold text-emerald-800 uppercase tracking-wider mb-1 flex items-center gap-1">
                              <FlaskConical size={12} /> Lab Session
                            </p>
                            <p className="text-sm text-emerald-700">
                              Pre-authorized — student joined via session code
                            </p>
                          </div>
                        )}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-black/[0.02] p-4 rounded-xl border border-black/5">
            ...
            <div className="relative">
              <NeumorphInput
                placeholder="Search inventory by name to add manually…"
                value={manualForm.manualSearch}
                onChange={(e) => setManualForm({ ...manualForm, manualSearch: e.target.value })}
                icon={<Search size={16} />}
                className="w-full"
              />
              {manualForm.manualSearch && (
                <div className="absolute z-10 w-full mt-1 bg-white border border-black/10 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                  {inventory.filter(i => i.name.toLowerCase().includes(manualForm.manualSearch.toLowerCase())).slice(0, 10).map(invItem => (
                    <button
                      key={invItem.id}
                      onClick={() => addNewItemToCart(invItem, setManualItems)}
      ...
      <NeumorphModal open={issueModal} onClose={() => setIssueModal(false)} title={`Issue Request #${selectedRequest?.id}`} size="lg">
        <div className="space-y-6">
          {selectedRequest?.lab_session_id && (
            <div className="bg-emerald-50 border border-emerald-200 p-3 rounded-xl text-sm flex items-center gap-2">
              <FlaskConical size={16} className="text-emerald-600 flex-shrink-0" />
              <span className="text-emerald-800"><strong>Lab Session Request</strong> — pre-authorized by faculty. Scan barcodes and issue directly.</span>
            </div>
          )}
          ...
          <div className="bg-black/[0.02] p-4 rounded-xl border border-black/5">
            ...
            <div className="relative">
              <NeumorphInput
                placeholder="Type to search inventory if scanner fails…"
                value={itemSearch}
              ...
              {itemSearch && (
                <div className="absolute z-10 w-full mt-1 bg-white border border-black/10 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                  {inventory.filter(i => i.name.toLowerCase().includes(itemSearch.toLowerCase())).slice(0, 10).map(invItem => (
                    <button
                      key={invItem.id}
                      onClick={() => addNewItemToCart(invItem, setAdjustedItems)}
                      className="w-full text-left px-4 py-3 hover:bg-primary/5 border-b border-black/5 flex justify-between items-center"
                    >
                      <span className="text-sm font-medium">{invItem.name} <span className="text-xs text-muted">({invItem.kind})</span></span>
                      <Plus size={16} className="text-primary" />
