...
export default function DashboardLayout() {
  ...
  // 1. Unified Fetch Function (Filtered by Room)
  const fetchPendingCount = useCallback(async () => {
    if (!isAdmin || !user?.room_id) return;
    try {
      // ⚡ STRICT FILTER: Only grab requests for THIS admin's room
  ...
  }, [isAdmin, user?.room_id]);

  // 2. Setup Socket Listeners & Initial Load
  useEffect(() => {
    if (!isAdmin) return;
    ...
    socket.on('new-request', (newReq) => {
      // ⚡ STRICT FILTER: Only trigger if the request belongs to THIS admin's room
      const reqRoomId = newReq.room_id || newReq.roomId;
      
      if (String(reqRoomId) === String(user.room_id)) {
    ...
    // Listen for updates (approvals/rejections) to recount badge
    socket.on('request-updated', (payload) => {
      const reqRoomId = payload.room_id || payload.roomId;
      if (String(reqRoomId) === String(user.room_id)) {
        fetchPendingCount(); 
      }
    });
  ...
  }, [isAdmin, user?.room_id, navigate, fetchPendingCount]);
